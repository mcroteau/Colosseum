...
public class UserRepo {
	...
	public User getByUsername(String username) {
		...
		try{
			...
			user = (User) qio.get(sql, new Object[] { username }, User.class);
		}catch(Exception ex){
			ex.printStackTrace();
		}
		return user;
	...
	public User getByUsernameAndUuid(String username, String uuid){
		...
		try{
			...
			user = (User) qio.get(sql, new Object[] {
					username,
					uuid
			}, User.class);
	...
	public boolean checkSaveSuperRole(long userId){
		Role role = roleRepo.find(Sigma.SUPER_ROLE);
	...
	public boolean checkSaveDefaultUserPermission(long userId){
		String permission = Sigma.USER_MAINTENANCE + userId;
	...
	public boolean removePlan(long id) {
		String sql = "update accounts set okay_plan_id = null, stripe_subscription_id = null where id = [+]";
		qio.delete(sql, new Object[] { id });
		return true;
	}

	public boolean updatePlan(User user) {
		String sql = "update accounts set stripe_id = [+], okay_plan_id = [+], stripe_subscription_id = [+] where id = [+]";
		qio.update(sql, new Object[] {
				user.getStripeId(),
				user.getOkayPlanId(),
	...
	public List<User> getOkayPlanList(long id) {
		String sql = "select * from accounts where okay_plan_id = [+]";
		List<User> people = (ArrayList) qio.getList(sql, new Object[]{id}, User.class);
		return people;
	}
}